...
├── scripts/                  # Verification and interactive testing scripts
├── data/
...
Plus:
- `tests/test_<module>.py` — Unit tests with pytest
- `scripts/test_<module>.py` — Interactive verification script
...
| **M7: Complete RAG** | ⏸️ | `src/rag/rag_pipeline.py` + API/CLI | `python scripts/query_rag.py "question"` |
...
3. Create verification script in `scripts/test_<name>.py`.
